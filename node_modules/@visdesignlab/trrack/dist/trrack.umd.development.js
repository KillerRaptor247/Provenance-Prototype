(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('lz-string'), require('mobx'), require('deep-diff')) :
  typeof define === 'function' && define.amd ? define(['exports', 'lz-string', 'mobx', 'deep-diff'], factory) :
  (global = global || self, factory(global.trrack = {}, global.LZString, global.mobx, global.DeepDiff));
}(this, (function (exports, lzString, mobx, deepDiff) { 'use strict';
  ...
  function getState(graph, node) {
    if (isRootNode(node) || isStateNode(node)) {
      return mobx.toJS(node.state);
    } // eslint-disable-next-line no-underscore-dangle


    var _state = mobx.toJS(graph.nodes[node.lastStateNode].state);

    var state = deepCopy(_state); // what is this for?

    node.diffs.forEach(function (diff) {
      deepDiff.applyChange(state, null, diff);
    });
    return state;
  }
  ...
  function defaultSerializer(obj) {
    if (!obj) return {};
    var str = JSON.stringify(mobx.toJS(obj), function (_, val) {
  ...
  var goToNode = /*#__PURE__*/mobx.action(function (graph, id) {
  ...
  var applyActionFunction = /*#__PURE__*/mobx.action(function (_graph, actionFn, currentState, // eslint-disable-next-line no-unused-vars
  ...
  var importState = /*#__PURE__*/mobx.action(function (graph, importedState) {
  ...
  function createStore(initState, serializer) {
    var obs = mobx.makeAutoObservable(createProvenanceGraph(serializer(initState)));
    return obs;
  }
  /**
   * @template T Represents the given state of an application as defined in initProvenance.
  ...
  function initProvenance(initialState, _opts) {
    ...
    var state = mobx.computed(function () {
      return deserializer(getState(graph, graph.nodes[graph.current]));
    });
    if (loadFromUrl) mobx.reaction(function () {
      return state.get();
    }, function (state) {
      var url = new URL(window.location.href);
      var params = new URLSearchParams(url.search);
      var stateEncodedString = lzString.compressToEncodedURIComponent(JSON.stringify(serializer(state)));
    ...
    return {
      ...
      addArtifact: mobx.action('Add Artifact Action', function (artifact, _id) {
      ...
      addAnnotation: mobx.action('Add Annotation Action', function (annotation, _id) {
        var id = graph.current;
        if (_id) id = _id;
        var node = graph.nodes[id];
      ...
      getAllArtifacts: function getAllArtifacts(_id) {
        ...
        if (isChildNode(node)) {
          return mobx.toJS(node.artifacts.customArtifacts);
        }
      ...
      getLatestArtifact: function getLatestArtifact(_id) {
        ...
        if (isChildNode(node)) {
          ...
          return mobx.toJS(arts[arts.length - 1]);
        }
      ...
      getAllAnnotation: function getAllAnnotation(_id) {
        ...
        if (isChildNode(node)) {
          return mobx.toJS(node.artifacts.annotations);
        }
      ...
      getLatestAnnotation: function getLatestAnnotation(_id) {
        ...
        if (isChildNode(node)) {
          ...
          return mobx.toJS(annotations[annotations.length - 1]);
        }
      ...
      setBookmark: mobx.action('Bookmark Action', function (id, bookmark) {
      ...
      exportState: function exportState(partial) {
        ...
        var compressedString = lzString.compressToEncodedURIComponent(JSON.stringify(exportedState));
      ...
      importState: function importState$1(s) {
        ...
        if (typeof s === 'string') {
          state = JSON.parse(lzString.decompressFromEncodedURIComponent(s) || '{}');
        } else {
      ...
      exportProvenanceGraph: function exportProvenanceGraph() {
        return JSON.stringify(mobx.toJS(graph));
      },
      importProvenanceGraph: mobx.action('Import Provenance Graph', function (g) {
        var importedGraph;
  ...
  function createAction(func) {
    ...
    var actionObject = function actionObject() {
      ...
      return {
        apply: mobx.action(function (state, label) {
  ...
  exports.initProvenance = initProvenance;
...
//# sourceMappingURL=trrack.umd.development.js.map
